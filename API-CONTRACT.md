...
- 첫 유효 방문은 그날 팩의 **지역 컬렉션**을 확정한다. 결과 카드는 방문한 단일 spot에
  묶지 않고 발급 당시 그 지역의 active versioned pool에서 고른다. 이후 같은 날 다른 지역을
  방문해도 이미 발급된 팩·결과·pool version은 바뀌지 않는다
- 특별 결과는 해당 지역 pool에서 아직 보유하지 않은 특별카드를 우선하며, 전부 보유한 뒤에만
  특별 중복을 허용한다. 일반 결과는 pool의 일반 후보에서 중복을 허용한다
...
현재 시행 중인 네 정책(`terms_of_use`·`privacy_policy`·`community_guidelines`·`location_terms`)이 모두 6개 locale의 HTTPS URL과 형식상 유효한 64자 lowercase SHA-256 메타데이터를 갖지 않으면 500 `INTERNAL`로 fail closed한다. 서버의 current 조회는 원격 문서 바이트를 다시 내려받지 않으므로, 실제 문서 바이트와 저장 hash의 일치는 발행 canary와 앱에서 문서를 열 때 검증한다. `support_url`은 로그인·연령 상태와 무관한 공개 고객지원 페이지다. 로컬 개발에서 미설정이면 `null`이지만 preview·production 서버는 `PUBLIC_SUPPORT_URL`이 없으면 환경 검증 단계에서 시작하지 않는다. production URL의 실제 값은 외부 배포 설정이다. 발행된 정책의 type·version·시행/발행시각과 locale별 URL·SHA-256은 수정·삭제할 수 없고 새 버전을 발행해야 한다. current set 전환은 service-role 전용 publication RPC만 허용하며, 이 RPC는 정책 전환 exclusive advisory lock을 정책 행보다 먼저 획득한다. 직접 `is_current` 변경은 DB trigger가 거부한다. placeholder URL·hash는 production 준비 완료로 간주하지 않는다. 앱은 정책과 지원 모두에 build-time exact HTTPS origin allowlist, 무리디렉션 최종 URL, credential 미전송, 10초 단일 deadline과 2MiB streaming 상한을 확인한다. 표시 텍스트는 262,144 UTF-16 코드 단위로 제한하고 HTML은 파싱 전에 256KiB 및 마크업 시작 4,096개 상한도 적용한다. 정책 문서는 실제 byte SHA-256까지 대조한 뒤 외부 URL을 재요청하지 않고, 검증된 동일 응답 바이트에서 추출·정규화한 텍스트를 앱 내 뷰어에 표시한다. 지원 페이지는 현재 API가 hash를 제공하지 않으므로 정확한 HTTPS URL·origin과 동일 응답만 검증하며 SHA-256 고정 대상이라고 표시하지 않는다. HTML은 표준 HTML 파서로 텍스트 노드만 추출하고 script·style·template 등 실행 또는 비본문 subtree를 제외하며, XML 전용 파서가 없는 `application/xhtml+xml`은 거부한다. 응답 가능한 지원 이메일·계정 삭제 URL과 실제 policy/support URL은 후보 빌드 전 외부 블로커다.
