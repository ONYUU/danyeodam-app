...
test("every Supabase CI job remaps before start and always destroys its stack", () => {
  const startCount = (workflow.match(/- run: corepack npm run db:start$/gmu) ?? []).length;
  const remapCount = (
    workflow.match(/- run: npm run ci:remap-supabase-ports$/gmu) ?? []
  ).length;
  const alwaysStopCount = (
    workflow.match(
      /- if: always\(\)\n\s+run: corepack npm run db:stop/gmu,
    ) ?? []
  ).length;
  ...
  for (const [label, job] of [
    ...
    const startIndex = job.indexOf("corepack npm run db:start");
    const alwaysStopIndex = job.indexOf(
      "- if: always()\n        run: corepack npm run db:stop",
    );
...
test("bonus HTTP E2E has no automatic CI bypass and requires explicit disposal", () => {
  ...
  assert.match(bonusE2e, /caller must run `corepack npm run db:stop`/u);
