...
export type BlockCursor = z.infer<typeof cursorPayloadSchema>;

function signature(payload: string, authUserId: string, secret: string): Buffer {
  return createHmac("sha256", secret)
    .update("danyeodam.blocks.v1\n", "utf8")
...
export function encodeBlockCursor(
  anchor: Omit<BlockCursor, "v">,
  authUserId: string,
  secret: string,
): string {
...
export function decodeBlockCursor(
  token: string,
  authUserId: string,
  secret: string,
): BlockCursor {
