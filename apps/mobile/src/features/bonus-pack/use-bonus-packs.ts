...
import { getBonusPackPage } from '@/api/bonus-packs-client';
import { useSessionDataRefresh } from '@/features/session-data/refresh-provider';
import type { SupportedLocale } from '@/i18n/locales';

export type BonusPacksLoadState =
...
export function useBonusPacks(input: {
  ...
  const [result, setResult] = useState<{
    requestKey: string;
    state: BonusPacksLoadState;
  } | null>(null);
  const state = useMemo<BonusPacksLoadState>(() => {
    ...
    void getBonusPackPage({
      ...
      setResult((current) => current?.requestKey === requestKey
        ...
        ? {
            ...
            state: {
              ...current.state,
              items: mergeBonusPacks(current.state.items, page.items),
              nextCursor: page.nextCursor,
              hasMore: page.hasMore,
              sealedCount: page.sealedCount,
              loadingMore: false,
